...
const envPath = path.join(cwd, ".env.staging");
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error(
    "Missing VITE_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY in .env.staging.",
  );
  console.error("");
  console.error("Add to .env.staging:");
  console.error("VITE_SUPABASE_URL=https://your-staging-project.supabase.co");
  console.error("SUPABASE_SERVICE_ROLE_KEY=your_staging_service_role_key");
  process.exit(1);
}

if (SUPABASE_URL.includes("YOUR_STAGING_PROJECT")) {
  console.error(
    "VITE_SUPABASE_URL in .env.staging is still set to the placeholder value. Replace it with your real staging Supabase URL.",
  );
  process.exit(1);
}

if (SUPABASE_SERVICE_ROLE_KEY === "your_staging_service_role_key") {
  console.error(
    "SUPABASE_SERVICE_ROLE_KEY in .env.staging is still set to the placeholder value. Replace it with your real staging service role key.",
  );
  process.exit(1);
}
...
console.log("🧪 Provisioning member in STAGING database");
...
console.log("✅ Member row upserted successfully in STAGING database.");
console.log(
  JSON.stringify(
    {
...
function loadEnvFile(filePath) {
  if (!fs.existsSync(filePath)) {
    console.error(`Could not find ${filePath}. Make sure .env.staging exists in the project root.`);
    process.exit(1);
  }
...
function printHelp() {
  console.log(`
🧪 Provision a closed-group member in the STAGING Supabase database.

This script provisions members in both Supabase Auth and public.members
for the staging database specified in .env.staging.

Usage:
  node scripts/provision-member-staging.mjs --email person@example.com [options]
...
Environment Variables Required (in .env.staging):
  VITE_SUPABASE_URL        - Staging database URL
  SUPABASE_SERVICE_ROLE_KEY - Staging database service role key

Example:
  node scripts/provision-member-staging.mjs \\
