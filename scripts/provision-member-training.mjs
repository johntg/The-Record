...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error(
    "Missing VITE_SUPABASE_URL_TRAINING or SUPABASE_SERVICE_ROLE_KEY_TRAINING. Add these to your local .env before running this script.",
  );
  console.error("");
  console.error("Add to .env:");
  console.error(
    "SUPABASE_SERVICE_ROLE_KEY_TRAINING=your_training_service_role_key",
  );
  process.exit(1);
}
...
if (SUPABASE_SERVICE_ROLE_KEY === "your_service_role_key") {
  console.error(
    "SUPABASE_SERVICE_ROLE_KEY_TRAINING is still set to a placeholder value in .env. Replace it with your real Supabase training service role/secret key before running this script.",
  );
  process.exit(1);
}
...
if (
  SUPABASE_SERVICE_ROLE_KEY ===
    (process.env.VITE_SUPABASE_ANON_KEY_TRAINING || "") ||
  SUPABASE_SERVICE_ROLE_KEY.startsWith("sb_publishable_")
) {
  console.error(
    "SUPABASE_SERVICE_ROLE_KEY_TRAINING is set to a publishable/anon key, not an admin key. Use the Supabase service role or secret key for admin scripts.",
  );
  process.exit(1);
}
...
console.log("🎓 Provisioning member in TRAINING database");
...
console.log("✅ Member row upserted successfully in TRAINING database.");
console.log(
  JSON.stringify(
    {
...
function printHelp() {
  console.log(`
🎓 Provision a closed-group member in the TRAINING Supabase database.

This script provisions members in both Supabase Auth and public.members 
for the TRAINING database specified by VITE_SUPABASE_URL_TRAINING.

Usage:
  node scripts/provision-member-training.mjs --email person@example.com [options]
...
Environment Variables Required (in .env):
  VITE_SUPABASE_URL_TRAINING          - Training database URL
  SUPABASE_SERVICE_ROLE_KEY_TRAINING  - Training database service role key

Example:
  node scripts/provision-member-training.mjs \\
    --email jwharford@gmail.com \\
    --name "John Harford" \\
    --role stake
`);
}
